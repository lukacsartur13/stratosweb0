...
# 20. Activation pass — 2026-08-15

An activation workstream, not an implementation one. No product feature was added, no
implementation was changed, and no file in `experiments/src`, `portal/src`, `netlify/` or
`supabase/` was edited. What follows is what was verified, what was applied, and what could
not be.

## 20.0 The starting state was not the stated one

The activation brief described the repository as: `HEAD` at `ec409f4`, implementation
uncommitted, not pushed, not deployed. **None of those four were true when this pass began.**

| Stated | Actual |
|---|---|
| `HEAD` = `ec409f4` | `HEAD` = `5e84feb` |
| implementation uncommitted | committed as `c3506e6` |
| not pushed | `origin/main` = `5e84feb`, zero ahead / zero behind |
| not deployed | deployed; `stratosweb1.netlify.app` carries it (recorded in `5e84feb`) |

So the two commit-and-stop steps of the brief were already satisfied by the previous
workstream, and nothing was committed or pushed in this pass. The working tree carries **no
feature work at all** — only tool and build residue, itemised in §20.6.

## 20.1 Diff audit

Audited `ec409f4..HEAD`, which is the whole of the implementation. It corresponds to the
completed-workstream report. Nothing unrelated is in it: 14 source files across
`experiments/src/full/mobile`, `portal/src`, `netlify/functions`, four test files and two
tsconfigs. **No asset, model or Blender file is committed in the range.**

Mobile Altimeter — every item confirmed against the source:

| Claim | Evidence |
|---|---|
| same real Altimeter GLB | one `models/stratos-altimeter.glb` request; unchanged since Aug 3 |
| one instrument scene | single `AltimeterInstrument` mounted once, sibling of the flow |
| seven states + departure | `hero, ascent, capabilities, summit, work, process, arrival` + `recede` |
| fixed pointer-transparent overlay | measured `position: fixed`, `pointer-events: none` |
| hero transition scroll-linked only | `HERO_TRAVEL = 0.6` screens from `HERO_HANDOVER = 0.26`; everything after is discrete |
| no terrain / mountains / clouds | no such request; `cloud-*` are section ids, not a subsystem |
| no HDR | no `.hdr/.exr/.env` request — environment is PMREM-baked in-page from flat emitters |
| no DRACO | no `draco` request on the mobile path |
| no new scroll listener | `ascent.ts` is **unchanged** in the range; the instrument adds none |
| no layout-feedback loop / per-frame geometry reads | no forced layout read on the instrument's path |
| instrument under editorial copy | overlay `z-index: 1`, `.mv-flow` `z-index: 2` — measured |
| exits before footer/Arrival | `recede` at opacity 0.003 past the `mv-end` marker — measured |

Portal — all fourteen surfaces present: Analytics Overview with previous-period comparison,
realtime, trend, acquisition, Top Pages, Landing Pages, devices, conversion funnel, aggregate
lead attribution, Command Center (`pages/overview.tsx`), pipeline, Proposal stage, private
notes, recorded-event timeline, System Health.

Security — all six confirmed:

* Google credentials are read only in `netlify/functions/`; grep for `GOOGLE_PRIVATE_KEY`,
  `GOOGLE_SERVICE_ACCOUNT_EMAIL`, `SUPABASE_SERVICE_ROLE_KEY` and `SUPABASE_SECRET_KEY` across
  `portal/src`, `experiments/src` and `assets/js` returns **nothing**;
* the numeric property id `15392224433` appears in **no** frontend source;
* `/api/portal-analytics` requires a bearer token, resolves the role from `profiles` rather
  than the JWT, and admits only `admin` / `super_admin`;
* the public tag strips PII-shaped fields by denylist (`email`, `phone`, `message`,
  `submission_id`, `user_id`, …) before anything is sent;
* lead data is aggregated in the Portal and never joined to a GA4 user;
* `/api/portal-health` returns booleans and state words only — `configured(v)` is
  `typeof v === 'string' && v.trim().length > 0`, never a value, host, path or length.

CSP needs no change: the browser calls `'self'` only, and Google is reached server-side.

## 20.2 Supabase

**Reviewed. Not applied. It cannot be applied from this environment.**

`supabase/migrations/20260814000100_lead_pipeline.sql` — migration plan:

| | |
|---|---|
| Types | `lead_status` gains value `proposal`, positioned `after 'qualified'` |
| Tables | `lead_notes` (`create table if not exists`) |
| Columns | none added to any existing table |
| Indexes | `lead_notes_lead_idx (lead_id, created_at desc)`; `activity_entity_idx (entity_type, entity_id, created_at desc)` |
| Policies | three, all on the new `lead_notes` table: select `is_staff()`, insert `is_admin() and author_id = auth.uid()`, delete `is_admin() and author_id = auth.uid()` |
| Functions | `log_lead_status_change()` — `security definer`, `set search_path = public` |
| Triggers | `leads_status_change` — `after update of status on leads` |
| Defaults | `lead_notes.id` `gen_random_uuid()`, `created_at` `now()` |
| Nullability | `lead_id`, `body`, `created_at` not null; `author_id` nullable by design |
| Foreign keys | `lead_id → leads(id) on delete cascade`; `author_id → profiles(id) on delete set null` |
| Existing data | **untouched.** No `update`, no `alter column`, no backfill |
| RLS | enabled and forced on the new table; no existing policy altered |

Dependencies verified present in earlier migrations: `leads`, `profiles`, `activity_logs`
(`20260801000100_schema.sql`), `is_staff()` / `is_admin()` (`20260801000200_rls.sql`), and
`lead_status` already carries `qualified`, so the `after 'qualified'` anchor resolves.

**Truly additive — confirmed.** No dropped column, no dropped table, no destructive type
conversion, no mass rewrite of lead data, no destructive rename. The two `drop` statements are
`drop policy if exists` and `drop trigger if exists`, both immediately recreated, both on
objects this migration owns. Enum `spam` is deliberately retained rather than removed.

Rollback: `drop trigger leads_status_change on leads; drop function log_lead_status_change();
drop table lead_notes; drop index activity_entity_idx;`. The added enum value cannot be
removed without a type rewrite — which is precisely why it was added rather than replaced. Any
lead already moved to `proposal` would need reassigning before such a rewrite.

One robustness note, reported and **not changed**: the enum block is wrapped in
`exception when others then null`, which swallows every failure, not only "already exists". If
the `alter type` ever failed for another reason the migration would continue silently and
`proposal` would be missing with no error. It is safe; it is just quiet.

**Why it was not applied.** No Supabase access exists in this environment: `supabase` and
`psql` are both absent, there is no `.env` (only `.env.example`), and no `SUPABASE_*` variable
is set in the shell. Nothing was faked and nothing was half-run. Post-application verification
— migration recorded, lead count unchanged, existing leads readable, Proposal storable, notes
storable, timeline readable, RLS intact — is therefore **not performed**, and §20.7 carries the
exact action required.

## 20.3 Google Cloud / GA4

**Not configured, and not configurable from here.** `gcloud` is absent, no service-account
JSON exists in the working copy, and no `GOOGLE_*` or `GA4_*` variable is set. Consequently:

* **no real Data API request was attempted** against property `15392224433`;
* Google project state, Data API enablement and service-account property access are all
  **unverified** — this pass has no evidence either way and does not claim any;
* the Portal correctly reports `NOT CONFIGURED`, which is the state the code is designed to
  land in and the state the deployed Portal is in now.

The architecture is unchanged and correct: GA4 → Data API → authenticated Netlify Function →
authenticated Portal. The browser never contacts Google for reporting.

## 20.4 The exact environment variables

Read from the implementation. These are the real names; none were invented.

```
GA4_PROPERTY_ID
→ the numeric Analytics property (15392224433) the Data API reports on
→ NON-SECRET
→ Netlify server environment (Functions scope)

GOOGLE_SERVICE_ACCOUNT_EMAIL
→ the service account's address, the JSON key file's `client_email`; the JWT `iss`
→ NON-SECRET (an identifier, not a credential)
→ Netlify server environment (Functions scope)

GOOGLE_PRIVATE_KEY
→ the service account's RS256 signing key, the JSON key file's `private_key`
→ SECRET — anyone holding it can read the property
→ Netlify server environment ONLY, Functions scope. Never in the repo, never in a
  browser bundle. Paste with the literal \n escapes; the function converts them.

GA4_PRODUCTION_HOSTS
→ optional. Comma-separated hostnames counted as Production by the hostName filter.
→ NON-SECRET
→ Netlify server environment. Defaults to `stratosweb.hu,www.stratosweb.hu`.
```

All three required names are also what `missingConfig()` reports and what
`/api/portal-health` reports as booleans. No value of any kind is recorded here.

## 20.5 Failure states, cache, and the environment filter

**Failure states.** The endpoint distinguishes two of the five named states explicitly and
collapses three. This is a deliberate, commented decision in the implementation, reported here
rather than changed:

| State | Behaviour |
|---|---|
| NOT CONFIGURED | `200 {configured:false, missing:[names]}` — distinct, and names the absent variables |
| SUCCESS | `200 {configured:true, data}` |
| NO DATA | `200` with zeroed metrics — a successful call over an empty period |
| ACCESS DENIED | collapsed into `502 UPSTREAM_FAILED` |
| API DISABLED / CONFIG ERROR | collapsed into `502 UPSTREAM_FAILED` |

Failures are **not** presented as `0` — they are a 502 with an error state in the UI, which is
the thing that most matters. But a reader cannot currently tell "the service account lacks
property access" from "the Data API is disabled" from the Portal alone; both are distinguished
only in the function log, which does record Google's own message (the mocked suite exercises
exactly this, including a `PERMISSION_DENIED … lacks access to property 15392224433` path).
Worth narrowing when credentials exist and the distinction becomes actionable.

**Cache** — actual TTLs, from `__cache`:

* historical reports: **5 minutes**, key `` `${range}|${environment}` `` — includes both the
  date range and the environment, so Production and staging cannot serve each other's payload;
* realtime: **60 seconds**, key `realtime`, single-shelf **and correctly so** — `hostName` is
  not a realtime dimension, so the realtime panel is whole-property by construction and
  declares `environmentFiltered: false`, which the UI surfaces;
* access token: cached until 60 s before expiry, and cleared on any failure;
* errors are **never** cached — the `catch` returns without a `set`;
* authentication and authorization both run **before** any cache read, so the cache cannot be
  used to bypass auth;
* per warm Lambda instance only, and the code says so rather than overclaiming.

Nine dashboard sections cost four Google calls, not nine.

**Production / staging.** `hostName` remains the distinction, unchanged, with `Production` the
default. What can be guaranteed today: the application-side `inListFilter` on the built-in
`hostName` dimension. What **cannot** be claimed: any GA4 Console property-level filter — none
was verified, because no GA4 access exists. §13's finding stands and is the most important
operational fact here: the live deploy is on `stratosweb1.netlify.app`, which the filter
correctly classifies as staging, so **Portal Analytics opening on Production will read zero
until the domain moves or `GA4_PRODUCTION_HOSTS` is set to include the netlify.app host.**

**Cross-check against GA4** — §10 of the brief — **not performed.** It requires a real request,
and none was made. No parity claim is made anywhere in this report.

## 20.6 Gates

Run against the working tree at `5e84feb`. Nothing was changed to obtain a result — no
timeout raised, no retry added, no worker count altered.

| Gate | Result |
|---|---|
| `typecheck` (portal + experiments) | pass |
| `validate:full` (clean + build + full suite) | **131 passed / 0 failed / 34 skipped** — the documented baseline, exactly |
| Portal auth + Analytics (mocked) + Health + leads + attribution | **359 passed / 0 failed** |
| route audit | 66 routes × 12 viewports = **792 checks, 0 failing, 0 broken links** |
| `scan:secrets` | clean — 680 files, 12 rules |
| `audit:conversion:check` | no CTA integrity failures |
| `audit:seo:check` | 72 documents, **0 failing**, 43 warnings |
| `draco:check` | matches three 0.171.0 |
| `fingerprint:check` | 72 pages, 25 assets, 0 unstamped |
| CSP | unchanged and sufficient — the browser calls `'self'`; Google is server-side |
| PII analytics scan | denylist confirmed active |
| real GA4 request | **not performed** — no credentials |

**Contention, reported not hidden.** Running `homepage-history` + `homepage-modality` +
`mobile-homepage-simple` together at default workers produced a different failing set on each
of two runs — first `the renderer is not requested at all when there is no WebGL`
(`mobile-430`), then two `keyboard focus stays in the layer, Escape closes it` (`desktop-webkit`,
`portrait-chromium`). Every one of them passes deterministically in isolation:

| Isolated run | Result |
|---|---|
| `mobile-homepage-simple.spec.ts --project=mobile-430 --workers=1` | 26 passed, 1 skipped, **0 failed** |
| `homepage-modality.spec.ts --workers=1` | 8 passed, **0 failed** |
| `homepage-modality.spec.ts --project=desktop-webkit --workers=1` | 2 passed |
| `homepage-modality.spec.ts --project=portrait-chromium --workers=1` | 2 passed |

Same signature as the documented `homepage-chrome` contention in §"The `npm test` failure":
the set is not stable between runs, and empty at one worker. The single-worker runs are
diagnostic evidence and are not how the gate was run.

**Working tree.** Still dirty, and deliberately so — none of it is feature work and none of it
should be committed: `.claude/settings.local.json` (local tool permissions), three regenerated
`_build/reports/*.json` audit outputs, `assets/blender/stratos-altimeter.blend` and a
`__pycache__/*.pyc`. The `.blend` was modified **Aug 14** while the runtime
`public/models/stratos-altimeter.glb` is unchanged from **Aug 3** — so the dirty Blender source
did not produce the shipped model and is unrelated working state, exactly the case §15 of the
brief says to exclude. Untracked: `experiments/.tmp-*` probes, `design-system/`, and report
JSONs. Separately, 172 `* 2.*` iCloud duplicate files exist on disk; all are matched by
`.gitignore:37` and none is tracked.

## 20.7 Preview package — mobile Altimeter

Captured at **390×844, DPR 1**, unedited, from the current source. Files in
`altimeter-preview/`: `01-hero`, `02-hero-transition`, `03-first-docked`, `04-mid-journey`,
`05-process`, `06-work`, `07-arrival`, `08-departure`, plus `09-journey-1x.webm` — one
unedited 1× pass, top to footer, over the full 11 757 px document — and `manifest.json` with
the measured state behind every frame.

The states are measurably distinct, which is the point of the set:

| Frame | scrollY | size | opacity | placement |
|---|---|---|---|---|
| hero | 0 | 335 px | 1.00 | centred, its own reserved band |
| hero transition | 354 | 107 px | 0.75 | in flight to the rail |
| first docked | 1 126 | 101 px | 0.74 | right rail, `data-settled` |
| mid-journey | 3 217 | 172 px | 0.51 | centred |
| process | 7 760 | 103 px | 0.70 | right rail |
| work | 3 658 | **88 px** | 0.58 | smallest and quietest, as authored |
| arrival | 10 727 | 257 px | 0.85 | centred |
| departure | 10 981 | — | **0.003** | gone, before the shared chrome |

Verified behaviours:

* **remains visible after the hero** — yes, at every stage measured;
* **does not cover text** — overlay `z-index: 1` under `.mv-flow` `z-index: 2`, measured at
  every frame; `05-process.png` shows a line of copy crossing the dial and staying fully
  legible;
* **fixed and pointer-transparent** — `position: fixed`, `pointer-events: none`, at every frame;
* **authored state switches do not jump** — sizes and opacities move monotonically through the
  set; the 1× recording is the unedited evidence;
* **`scrollTo(scrollHeight)` reaches departure** — yes, opacity 0.003;
* **menu hides / deprioritises the instrument** — measured on the built site: 0.593 closed →
  **0.013 open** → 0.566 on Escape. The overlay is `aria-hidden="true"` throughout, correctly,
  being decorative;
* **history Back restoration** — scroll position restored within tolerance and the instrument
  returns to a live, non-hanging state.

**One finding, and it is not the instrument.** A **cold load** directly to a fragment —
`full.html#stage-process` — leaves the page at `scrollY: 0` when the target is at 7 652. The
instrument is *not* left hanging: it is correctly in the hero state, `data-settled`,
`data-ready`, opacity 1, consistent with the scroll position it is actually at. The defect is
that the document does not honour the fragment on a cold load, because the mobile homepage
mounts client-side and the element does not exist when the browser attempts the anchor scroll;
nothing re-applies it afterwards. Same-document navigation is **fine** — setting
`location.hash` lands at 7 644 against a 7 652 target, so every in-page anchor and menu link
works. This is out of scope for this workstream and is not caused by it: the range does not
touch section structure, ids, routing or hash handling, and `ascent.ts` is unchanged. Recorded
for a future pass.

## 20.8 Final status

**Supabase** — migration reviewed in full and confirmed truly additive; **not applied**;
validation **not performed** (no access from this environment).

**Google Cloud** — Data API enablement **unknown**; service account **missing** from this
environment; property access **unverified**.

**Netlify** — required names are `GA4_PROPERTY_ID`, `GOOGLE_SERVICE_ACCOUNT_EMAIL`,
`GOOGLE_PRIVATE_KEY` (secret), and optionally `GA4_PRODUCTION_HOSTS`; **not configured**.

**GA4** — real request **not performed**; property access **unverified**; production/staging
distinction is application-side on `hostName` only, with **no** verified Console filter; data
comparison **not performed**, and no parity is claimed.

**Portal** — Analytics **not configured**, falling back correctly; pipeline **blocked by the
unapplied migration** (Proposal and notes fail as handled paths, timelines show arrival only).

**Mobile** — persistent Altimeter **reviewed and ready**; full 390×844 package captured; every
checklist behaviour verified; one out-of-scope cold-load fragment finding recorded.

### The external actions, in order

1. **Apply the migration.** Supabase Dashboard → SQL Editor → paste
   `supabase/migrations/20260814000100_lead_pipeline.sql` → Run. Or, with the CLI installed and
   the project linked: `supabase db push`. Then confirm the lead count is unchanged, an
   existing lead still opens, a lead moves to Proposal, a note saves, and the timeline shows
   the status change.
2. **Create the Google service account.** A Google Cloud project; enable the **Google
   Analytics Data API**; create a service account and a JSON key; in GA4 Admin → Property
   Access Management add that `client_email` to property `15392224433` with **Viewer**.
3. **Set the Netlify variables** — the four names in §20.4, Functions scope, values never in
   the repo.
4. **Decide the environment default.** Until `stratosweb.hu` cuts over from Wix, either accept
   that Production reads zero, or set `GA4_PRODUCTION_HOSTS` to include
   `stratosweb1.netlify.app`.

Only then do §§6, 9, 10 and 12 of the activation brief — the real GA4 request, the live UI
validation, the GA4 cross-check and the pipeline workflow test — become performable.

---

EXTERNAL ACTIVATION REQUIRED BEFORE PREVIEW
