-- =============================================================================
-- STRATOS SALES PIPELINE — STEP 2 of 2: tables, policies, triggers, functions
--
-- RUN AFTER STEP 1 HAS COMMITTED.
--
-- This is `supabase/migrations/20260814000100_lead_pipeline.sql` followed by
-- `supabase/migrations/20260816000100_revenue_operations.sql`, verbatim, with
-- only the two `ALTER TYPE ... ADD VALUE` statements removed because step 1
-- already ran them. Nothing else is changed, added or reordered.
--
-- WHAT THIS DOES NOT DO
-- ---------------------
-- No DROP TABLE. No TRUNCATE. No DELETE. No ALTER ... DROP COLUMN. No data
-- migration of any kind. Every `drop` below is `drop trigger if exists` or
-- `drop policy if exists` immediately followed by the `create` that replaces it,
-- which is how a migration is made re-runnable. `leads` is not modified by a
-- single statement — not one column, not one row.
--
-- Every added column is nullable or defaulted, so every existing query keeps
-- working against tables that have only gained things.
--
-- The whole script runs as ONE transaction: if any statement fails, nothing is
...
-- (moved to step 1 — see apply-01-enums.sql)
