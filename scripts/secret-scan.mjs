 ...
 */
/*
 * THE BOUNDARY, stated so it can be argued with.
 *
 * A path belongs here when EVERY byte in it is written by a test runner or by
 * the gate, and none of it is ever typed by a person. That is a narrow rule and
 * it is deliberately not "anything under `_build/reports`": that directory is
 * mostly hand-written analysis, and a real key pasted into one of those reports
 * has to stay a finding. `phase9-dependency-audit.md` is an authored report
 * that quotes this scanner's own rules, and it is exempted BY NAME above rather
 * than by living in the same tree.
 *
 * Every entry below is a Playwright or gate JSON artefact. They are the only
 * files in the repository that can quote an arbitrary source line back at the
 * scanner, because that is what a failure report is for. The first four are the
 * live ones — regenerated by any gate run or by `npm test` — and the last two
 * are frozen reports from earlier workstreams, listed for the same reason
 * rather than because they currently match anything.
 */
const GENERATED_TREES = [
  ...
  // The WebGL suite's default JSON report, and the exact sibling of the line
  // above: `playwright.full.config.ts` writes here when the gate does not
  // override the path. Missing from the original fix, and the same cascade —
  // a failing spec that names a rule in order to forbid it would be quoted
  // into this file and turn every later run red.
  '_build/reports/regression-harness/last-full-run.json',
  // This workstream's targeted stress reports. Playwright JSON, from the lead
  // suite — which is the suite whose source names the rules.
  '_build/reports/lead-silent-drop/stress',
  // Frozen Playwright JSON from earlier workstreams. Machine-written in full.
  '_build/reports/mobile-test-reconciliation',
  '_build/reports/webkit-navigation/suite',
];
