...
const Q = [
  // ===== KÖZÖS: alapadatok =====
  {t:'text',    q:'Mi a vállalkozás neve?', req:1, id:'cegnev'},
  ...
  {t:'text',    q:'Ki tölti ki a kérdőívet? (név, pozíció)', id:'kitolto'},
...
const CUT_NOTE = '[…a válaszok itt megszakadtak]';
...
async function renderDone(){
  ...
  if(lead.name.length < 2){ finish('invalid', 'Kérjük, add meg a vállalkozás vagy a kapcsolattartó nevét.'); return; }
  if(!EMAIL_RE.test(lead.email)){ finish('invalid', 'Ez az e-mail cím nem tűnik helyesnek.'); return; }
  ...
  if(res && res.status === 422){
    ...
    finish('invalid', first || 'Kérjük, ellenőrizd a megadott adatokat.');
    return;
  }
  finish('error');
}
