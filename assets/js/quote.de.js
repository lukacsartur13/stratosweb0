...
const CUT_NOTE = '[…die Antworten wurden hier abgeschnitten]';
...
async function renderDone(){
  ...
  if(lead.name.length < 2){ finish('invalid', 'Bitte gib den Firmen- oder Ansprechpartnernamen an.'); return; }
  if(!EMAIL_RE.test(lead.email)){ finish('invalid', 'Diese E-Mail-Adresse sieht nicht richtig aus.'); return; }
  ...
  if(res && res.status === 422){
    ...
    finish('invalid', first || 'Bitte überprüfe die eingegebenen Daten.');
    return;
  }
  finish('error');
}
