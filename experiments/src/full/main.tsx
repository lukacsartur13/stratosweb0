...
// notice anything missing rather than after — and, as the note at the bottom
// of this list records, to be things only a person can do:
//
...
//   keydown       space, Page Down, Tab. This is also what covers a keyboard
//                 visitor reaching the skip link: you cannot arrive at it
//                 without pressing Tab, and Tab is a keydown.
//   scroll        the backstop, for scrolls none of the above preceded —
//                 momentum, a fragment link, an assistive technology.
//
// `focusin` WAS on this list, and taking it off is the whole of a defect worth
// recording. It looked like the right way to catch a keyboard visitor, and it
// is the one event here that a *script* fires as readily as a person:
// `assets/js/consent.js` moves focus into the cookie banner 60 ms after
// showing it, so on any host where that banner appears — which is every host
// except localhost, because `analytics.js` gates itself on `allowHosts` — the
// journey started on every cold load, with the visitor having done nothing at
// all. It cost nothing locally and 489 ms of Total Blocking Time on the
// deployed site, where it read as the deferral simply not working. Measured
// against https://stratosweb1.netlify.app/en/: TBT 498 ms with `focusin`
// armed, against 9 ms on the same build locally, where no banner renders.
//
// The lesson generalises past this one listener: a trigger that means "the
// visitor is here" must not be an event the page can fire at itself.
// `event.isTrusted` does not help — a focus event caused by `element.focus()`
// is trusted.
//
